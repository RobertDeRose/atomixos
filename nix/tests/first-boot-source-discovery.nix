...
nixos-lib.runTest {
  ...
  testScript = ''
    ...
    gateway.succeed("cat >/tmp/bootstrap-apply-test <<'EOF'\n#!/usr/bin/env bash\nset -euo pipefail\nsystemctl restart quadlet-sync.service\nif systemctl list-unit-files lan-gateway-apply.service >/dev/null 2>&1; then\n  systemctl restart lan-gateway-apply.service\nfi\nif systemctl list-unit-files provisioned-firewall-inbound.service >/dev/null 2>&1; then\n  systemctl restart provisioned-firewall-inbound.service\nfi\nEOF\nchmod +x /tmp/bootstrap-apply-test")
    gateway.succeed("cat >/tmp/bootstrap-reload-test <<'EOF'\n#!/usr/bin/env bash\nset -euo pipefail\nsystemctl try-restart atomixos-bootstrap.service\nEOF\nchmod +x /tmp/bootstrap-reload-test")
    gateway.succeed("env PATH=/testbin:/run/current-system/sw/bin ATOMIXOS_TEST_LIST_APPLY_SERVICES=1 /tmp/bootstrap-apply-test >/tmp/reapply-runtime.out 2>&1")
    gateway.succeed("env PATH=/testbin:/run/current-system/sw/bin /tmp/bootstrap-reload-test >>/tmp/reapply-runtime.out 2>&1")
