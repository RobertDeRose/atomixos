...
nixos-lib.runTest {
  ...
  testScript = ''
    ...
    gateway.succeed("cat > /testbin/forensic-log <<'EOF'\n#!/usr/bin/env bash\nprintf '%s\n' \"$*\" >>/test-state/forensic.log\nEOF\nchmod +x /testbin/forensic-log")
    ...
    gateway.succeed("env PATH=/testbin:/run/current-system/sw/bin ATOMIXOS_CONFIG_ROOT=/tmp/config-roots/fresh-usb-fallback ATOMIXOS_FIRST_BOOT_SENTINEL=/tmp/bootcases/fresh-usb-fallback/sentinel ATOMIXOS_FORENSICS_RAUC_STATE_DIR=/tmp/bootcases/fresh-usb-fallback/forensics ATOMIXOS_BOOT_SLOT=boot.a ATOMIXOS_USB_SEARCH_DIRS='/test-usb' ATOMIXOS_BOOTSTRAP_HOST=127.0.0.1 ATOMIXOS_BOOTSTRAP_PORT=18080 ATOMIXOS_INITRD_MARKER=/etc/atomixos/fresh-flash ATOMIXOS_BOOT_CONFIG_PATH=/boot/config.toml first-boot")
    ...
    gateway.succeed("env PATH=/testbin:/run/current-system/sw/bin ATOMIXOS_CONFIG_ROOT=/tmp/config-roots/reprovision-skips-boot ATOMIXOS_FIRST_BOOT_SENTINEL=/tmp/bootcases/reprovision-skips-boot/sentinel ATOMIXOS_FORENSICS_RAUC_STATE_DIR=/tmp/bootcases/reprovision-skips-boot/forensics ATOMIXOS_BOOT_SLOT=boot.a ATOMIXOS_USB_SEARCH_DIRS='/test-usb' ATOMIXOS_BOOTSTRAP_HOST=127.0.0.1 ATOMIXOS_BOOTSTRAP_PORT=18080 ATOMIXOS_INITRD_MARKER=/etc/atomixos/fresh-flash ATOMIXOS_BOOT_CONFIG_PATH=/boot/config.toml first-boot")
    ...
    gateway.succeed("rm -rf /test-usb/usb-a/* /test-usb/usb-b/*")
    gateway.succeed("ATOMIXOS_CONFIG_ROOT=/tmp/bootstrap-root ATOMIXOS_FIRST_BOOT_SENTINEL=/tmp/bootstrap-sentinel ATOMIXOS_FORENSICS_RAUC_STATE_DIR=/tmp/bootstrap-forensics ATOMIXOS_BOOT_SLOT=boot.a ATOMIXOS_USB_SEARCH_DIRS='/test-usb' ATOMIXOS_BOOTSTRAP_HOST=127.0.0.1 ATOMIXOS_BOOTSTRAP_PORT=18080 PATH=/testbin:/run/current-system/sw/bin first-boot >/tmp/bootstrap-case.log 2>&1 & echo $! >/tmp/bootstrap-case.pid")
    gateway.wait_until_succeeds("ss -tln | grep '127.0.0.1:18080'", timeout = 60)
    gateway.succeed("curl -fsS -H 'X-Config-Filename: config.toml' --data-binary @/tmp/config-template.toml http://127.0.0.1:18080/api/config >/tmp/bootstrap-api.json")
    gateway.wait_until_succeeds("test -f /tmp/bootstrap-root/config.toml", timeout = 60)
    gateway.succeed("for _ in $(seq 1 60); do if test -f /tmp/bootstrap-sentinel; then exit 0; fi; if ! kill -0 $(cat /tmp/bootstrap-case.pid) 2>/dev/null; then echo 'bootstrap process exited unexpectedly' >&2; test -f /tmp/bootstrap-case.log && cat /tmp/bootstrap-case.log >&2; test -f /test-state/forensic.log && cat /test-state/forensic.log >&2; exit 1; fi; sleep 1; done; echo 'bootstrap sentinel did not appear' >&2; test -f /tmp/bootstrap-case.log && cat /tmp/bootstrap-case.log >&2; test -f /test-state/forensic.log && cat /test-state/forensic.log >&2; exit 1")
    gateway.wait_until_succeeds("test ! -e /proc/$(cat /tmp/bootstrap-case.pid)", timeout = 60)
    gateway.succeed("python3 - <<'PY'\nimport json\nfrom pathlib import Path\npayload = json.loads(Path('/tmp/bootstrap-api.json').read_text())\nassert payload['ok'] is True, payload\nPY")
    gateway.succeed("grep 'bootstrap-console --result start' /test-state/forensic.log")
    gateway.succeed("grep 'bootstrap-console --result ok' /test-state/forensic.log")
    ...
    gateway.fail("env PATH=/testbin:/run/current-system/sw/bin ATOMIXOS_CONFIG_ROOT=/tmp/prod-missing-root ATOMIXOS_FIRST_BOOT_SENTINEL=/tmp/prod-missing-sentinel ATOMIXOS_FORENSICS_RAUC_STATE_DIR=/tmp/prod-missing-forensics ATOMIXOS_BOOT_SLOT=boot.a ATOMIXOS_USB_SEARCH_DIRS='/test-usb' ATOMIXOS_BOOTSTRAP_HOST=127.0.0.1 ATOMIXOS_BOOTSTRAP_PORT=18080 first-boot")
    gateway.succeed("test ! -f /tmp/prod-missing-sentinel")
    gateway.succeed("test ! -f /test-state/rauc.log")
    gateway.succeed("grep 'failed --slot boot.a --reason provisioning-missing' /test-state/forensic.log")
    ...
    gateway.succeed("env PATH=/testbin:/run/current-system/sw/bin ATOMIXOS_CONFIG_ROOT=/tmp/dev-fallback-root ATOMIXOS_FIRST_BOOT_SENTINEL=/tmp/dev-fallback-sentinel ATOMIXOS_FORENSICS_RAUC_STATE_DIR=/tmp/dev-fallback-forensics ATOMIXOS_BOOT_SLOT=boot.a ATOMIXOS_USB_SEARCH_DIRS='/test-usb' ATOMIXOS_BOOTSTRAP_HOST=127.0.0.1 ATOMIXOS_BOOTSTRAP_PORT=18080 ATOMIXOS_DEV_ENABLE_SSH_WAN=1 first-boot")
