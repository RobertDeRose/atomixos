...
# 6. Verifies the install switched the primary slot without relying on slot-local forensic storage
#
...
nixos-lib.runTest {
  ...
  testScript = ''
    ...
    gateway.log("RAUC update install test passed — bundle installed to inactive slot and primary switched to B")
  '';
}
