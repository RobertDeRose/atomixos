...
grep -F '[tasks.check]' "$TEST_REPO/mise.toml"
grep -F 'run = "hk check -a"' "$TEST_REPO/mise.toml"
if grep -F '[tasks."nix:check"]' "$TEST_REPO/mise.toml"; then
	echo "unexpected duplicate nix:check task in mise configuration" >&2
	exit 1
fi
