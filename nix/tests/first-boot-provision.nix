...
let
  ...
  provisionCli = python3.pkgs.buildPythonApplication {
    pname = "first-boot-provision";
    version = "0.1.0";
    pyproject = true;
...
nixos-lib.runTest {
  ...
  nodes.gateway =
    { ... }:
    {
      ...
      environment.variables.ATOMIXOS_ALLOW_UNSAFE_CONFIG_ROOT = "1";
  ...
  testScript = ''
    if True:
        ...
        gateway.succeed("""cat > /tmp/assert-api-job <<'PY'
    import json
    import sys
    import time
    ...
    base_url, response_path, expected_state = sys.argv[1:4]
    response = json.loads(Path(response_path).read_text())
    assert response['state'] in {'submitted', 'running'}, response
    job_id = response['job_id']
    for _ in range(120):
        with urllib.request.urlopen(f'{base_url}/api/jobs/{job_id}', timeout=2) as handle:
            job = json.loads(handle.read().decode())
        if job['state'] in {'succeeded', 'failed'}:
            assert job['state'] == expected_state, job
            Path(response_path + '.job').write_text(json.dumps(job))
            break
        time.sleep(0.25)
    else:
    ...
    PY""")
        ...
        gateway.succeed("grep 'AAAAC3NzaC1lZDI1NTE5AAAAIFGTDzwiQNe3nwhmg/G81QDhQBbpgOyvrKXeYnQHYOUd' /tmp/operator-admin-root/ssh-authorized-keys/operator")
        ...
        gateway.succeed("grep '^try-restart atomixos-bootstrap.socket$' /tmp/lan-apply/systemctl.log")
        gateway.succeed("grep '^try-restart atomixos-bootstrap.service$' /tmp/lan-apply/systemctl.log")
        ...
        gateway.succeed("grep '^stop obsolete.service$' /tmp/quadlet-sync/systemctl.log")
        ...
        gateway.succeed("cat > /tmp/apply-users-test/users.json <<'EOF'\n{\"admin\": {\"isAdmin\": true, \"ssh_key\": \"ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIAle55IgGqvF1AHaqsn09rJ013fHiGrgjLOX/B40qPsp admin@test\"}, \"operator\": {\"isAdmin\": true, \"ssh_key\": \"ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIPNjUd40YE0a8215r2XV0YNoDFRDSpbpO9NeC70wx2gi op@test\"}, \"viewer\": {\"isAdmin\": false, \"ssh_key\": \"\"}}\nEOF")
        gateway.succeed("printf 'ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIPNjUd40YE0a8215r2XV0YNoDFRDSpbpO9NeC70wx2gi op@test\n' >/tmp/apply-users-test/ssh-authorized-keys/operator && chmod 0600 /tmp/apply-users-test/ssh-authorized-keys/operator")
        ...
        gateway.succeed("curl -fsS -F \"bootstrap_token=$(cat /tmp/bootstrap-apply-token.txt)\" -F config_file=@/tmp/config.toml http://127.0.0.1:18080/apply >/tmp/bootstrap-response.html")
        ...
        gateway.succeed("test \"$(curl -sS -o /tmp/bootstrap-invalid-response.html -w '%{http_code}' -F \"bootstrap_token=$(cat /tmp/bootstrap-invalid-token.txt)\" -F config_file=@/tmp/invalid-config.toml http://127.0.0.1:18080/apply)\" = 400")
        ...
        gateway.succeed("curl -fsS -F \"bootstrap_token=$(cat /tmp/bootstrap-zstd-token.txt)\" -F config_file=@/tmp/config-dot.tar.zstd http://127.0.0.1:18080/apply >/tmp/bootstrap-zstd-upload-response.html")
        ...
        gateway.succeed("grep 'authentication required' /tmp/auth-unauth-response.json")
        ...
        gateway.succeed("grep '^404$' /tmp/auth-apply-code")
        gateway.succeed("curl -s -o /tmp/auth-generate-response.html -w '%{http_code}' --data-urlencode 'ssh_keys=ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIGfm/RhPyisFyBAwigDt2AnnOU1fMAVk4XaCj3S/k/3Z admin@example' http://127.0.0.1:18081/generate > /tmp/auth-generate-code")
        gateway.succeed("grep '^404$' /tmp/auth-generate-code")
        ...
        gateway.succeed("grep 'expired' /tmp/auth-replay-response.json")
        ...
        # Fresh first-boot programmatic API upload does not require the Boot UI CSRF token.
        ...
        gateway.succeed("python3 /tmp/assert-api-job http://127.0.0.1:18081 /tmp/auth-fresh-response.json succeeded")
        gateway.succeed("kill $(cat /tmp/auth-bootstrap.pid)")
        ...
        gateway.succeed("python3 /tmp/assert-api-job http://127.0.0.1:18082 /tmp/atomic-apply-response.json succeeded")
        ...
        # CLI import also uses candidate promotion and rolls back on activation failure.
        ...
        gateway.succeed("sha256sum /tmp/cli-atomic-root/config.toml | awk '{print $1}' > /tmp/cli-atomic-hash-before")
        gateway.fail("first-boot-provision import /tmp/no-health-config.toml /tmp/cli-atomic-root >/tmp/cli-atomic-import.out 2>/tmp/cli-atomic-import.err")
        gateway.succeed("grep 'activation failed: placeholder.service' /tmp/cli-atomic-import.err")
        gateway.succeed("test -f /tmp/cli-atomic-root/config.toml")
        gateway.succeed("sha256sum /tmp/cli-atomic-root/config.toml | awk '{print $1}' > /tmp/cli-atomic-hash-after")
        gateway.succeed("diff /tmp/cli-atomic-hash-before /tmp/cli-atomic-hash-after")
        gateway.succeed("grep '\"guest\"' /tmp/cli-atomic-root/managed-users.json")
        gateway.succeed("test ! -d /tmp/cli-atomic-root-rollback")
        ...
        gateway.succeed("curl -fsS -H 'Content-Type: text/plain' -H \"X-Atomicnix-Nonce: $(cat /tmp/reapply-invalid-nonce.txt)\" -H \"X-Atomicnix-Signature: $(cat /tmp/reapply-invalid-sig-b64.txt)\" --data-binary @/tmp/invalid-config.toml http://127.0.0.1:18083/api/config > /tmp/reapply-invalid-response.json")
        ...
        # Raw API re-apply rolls back when the activation hook fails.
        ...
        gateway.succeed("python3 - <<'PY'\nimport sys, os\nfrom pathlib import Path\nos.environ['ATOMIXOS_CONFIG_SCHEMA'] = '${provisionCli}/share/atomixos/config.schema.json'\nsys.path.insert(0, '${provisionCli}/${python3.sitePackages}')\nfrom atomixos_provision.activation import recover_config_root\nrecover_config_root(Path('/tmp/recover-active-root'))\nassert Path('/tmp/recover-active-root/config.toml').read_text() == 'rollback\\n'\nassert not Path('/tmp/recover-active-root-rollback').exists()\nassert not Path('/tmp/recover-active-root.atomixos-promotion-pending').exists()\nPY")
        ...
        gateway.succeed("rm -rf /tmp/cleanup-root /tmp/cleanup-root-rollback && mkdir -p /tmp/cleanup-root /tmp/cleanup-root-rollback")
        gateway.succeed("touch /tmp/cleanup-root/config.toml /tmp/cleanup-root-rollback/config.toml")
        gateway.succeed("python3 - <<'PY'\nimport sys, os\nfrom pathlib import Path\nos.environ['ATOMIXOS_CONFIG_SCHEMA'] = '${provisionCli}/share/atomixos/config.schema.json'\nsys.path.insert(0, '${provisionCli}/${python3.sitePackages}')\nfrom atomixos_provision.activation import cleanup_rollback\ncleanup_rollback(Path('/tmp/cleanup-root'))\nassert not Path('/tmp/cleanup-root-rollback').exists()\nPY")
