...
- [ ] T002 Review existing watchdog module, boot-count script, RAUC rollback docs, and VM tests
...
- [ ] T010 Inspect existing `modules/watchdog.nix` option structure
- [ ] T011 Add or refine `atomixos.watchdog.*` options for hardware enforcement
- [ ] T012 Keep VM, development, and Rock64 defaults disabled unless explicitly enabled
- [ ] T013 Add module assertions or guards for unsupported hardware contexts if needed

## Rock64 Systemd Enforcement

- [ ] T020 Render `RuntimeWatchdogSec=30s` when hardware enforcement is enabled
- [ ] T021 Render `RebootWatchdogSec=10min` when hardware enforcement is enabled
- [ ] T022 Confirm Rock64 watchdog driver/device availability in the target image
- [ ] T023 Ensure disabled enforcement leaves local recovery and VM boot unchanged

## Rollback Integration

- [ ] T030 Review watchdog boot-count recording path
...
- [ ] T033 Document U-Boot `BOOT_*_LEFT` hardware assumptions and custom VM simulation differences

## Tests And Validation

- [ ] T040 Add Nix evaluation or module tests for watchdog option defaults
- [ ] T041 Add or update VM assertions for rendered systemd manager settings where applicable
